...
public class BD extends javax.swing.JPanel {
    ...
    //metodo Threshold modificado como fue explicado por profesor arnoldo
 ...
 */            switch(curState){
                 ...
                 case 3://temperatura, !iluminacion y !humedad
                  ...
                  case 5:
                      //se agrega a la lista de detecciones  los registros que llegaron a este estado
 ...
 //       }  
        ...
        updateAvg();  //sacamos promedio
        FirstIndex++;//movemos los indices de la ventana
        LastIndex++;   
        }
    ...
    //Redondear los numeros a n digitos.
    public double redondear(double doble,int digitos){
    int cifras=(int) Math.pow(10,digitos);
    return Math.rint(doble*cifras)/cifras;
	}
    
    //Si la coneccion de BD no es correcta, se usa este metodo para cmabiar los datos e intentar denuevo
    ...
    public boolean coneccionBD(String s1, String s2){
        try{
            if(s1.equals("update"))    St.executeUpdate(s2);//si ejecutara un update/insert
            else                       Rs=St.executeQuery(s2);//delo contrario solo cargamos ResulSet
        }catch(Exception e){JOptionPane.showMessageDialog(GUI.f,"Error en consulta:\n Coneccion a Base de Datos");
                             return false;}
        return true;
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
       if(exec==null){//si no se esta ejecutando
        exec=new Thread(new Runnable(){//creamos nuevo hilo
            public void run(){
                init();
                //checa que tamano ventana y query no esten vacias
                if(jFormattedTextField1.getText().isEmpty()||jTextField1.getText().isEmpty())
                    JOptionPane.showMessageDialog(GUI.f,"Datos Incorrectos!","Error", JOptionPane.OK_OPTION);
                else {
                ...
                exec=null;//se desecha el hilo
             }});
        exec.setPriority(Thread.MAX_PRIORITY);//le ponemos maxima prioridad
        exec.start();//empezamos el hilo
       }
       else{//si ya se esta ejecutando, mostramos mensaje.
            JOptionPane.showMessageDialog(null,"Executing ... ","Excecuting",2);
